...
import type { Route } from "./+types/applicant-subjects";
import { sessionCookie, type AdminSession } from "../lib/session";

export function meta({}: Route.MetaArgs) {
  return [{ title: "Abituriyent Fənləri — İmtahanVer Admin" }];
}
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  try {
    const backendUrl = search
      ? `http://backend:80/api/adminapi/applicant-subjects?search=${encodeURIComponent(search)}`
      : "http://backend:80/api/adminapi/applicant-subjects";
    ...
    return { subjects: data.success ? data.data : [], session, search };
  } catch (err) {
    console.error("Applicant subjects loader error:", err);
...
export async function action({ request }: Route.ActionArgs) {
  ...
  try {
    if (intent === "create") {
      const res = await fetch("http://backend:80/api/adminapi/applicant-subjects", {
        method: "POST",
        headers,
        body: JSON.stringify({
          title: formData.get("title"),
          identify: formData.get("identify") || null,
        }),
      });
      const data = await res.json();
    ...
    if (intent === "update") {
      ...
      const res = await fetch(`http://backend:80/api/adminapi/applicant-subjects/${id}`, {
        method: "PUT",
        headers,
        body: JSON.stringify({
          title: formData.get("title"),
    ...
    if (intent === "reorder") {
      ...
      const res = await fetch("http://backend:80/api/adminapi/applicant-subjects/reorder", {
        method: "PUT",
        headers,
...
export default function ApplicantSubjectsPage() {
  const { subjects, search } = useLoaderData<typeof loader>();
  ...
  const handleDragEnd = (e: React.DragEvent) => {
    ...
    fd.append("ids", JSON.stringify(localSubjects.map((s) => s.id)));
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between bg-white border border-gray-150 rounded-2xl p-6 shadow-sm">
        <div>
          <h2 className="text-lg font-bold text-gray-900">Abituriyent Fənləri</h2>
          <p className="text-xs text-gray-500 mt-1">Abituriyent imtahanı fənlərinin sıralanması və idarəedilməsi. Sürüşdürərək sıralamanı dəyişə bilərsiniz.</p>
        </div>
        <button
          onClick={() => { setSelectedSubject(null); setShowAddModal(true); }}
      ...
      <p className="text-xs font-medium text-gray-450 uppercase tracking-wider">
        Tapılan fənn sayı: <strong className="text-gray-900">{localSubjects.length}</strong>
      </p>
      ...
      <div className="space-y-3">
        {localSubjects.map((subject, index) => (
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => { setSelectedSubject(subject); setShowEditModal(true); }}
                title="Redaktə et"
        ...
        {localSubjects.length === 0 && (
          <div className="py-20 text-center bg-white border border-gray-150 rounded-2xl">
            ...
            <p className="mt-3 text-sm text-gray-400 font-semibold">Fənn tapılmadı</p>
          </div>
        )}
      </div>
          ...
          <div className="bg-white border border-gray-150 rounded-2xl max-w-md w-full p-6 shadow-2xl animate-in zoom-in duration-200">
            ...
            <Form method="post" className="space-y-4">
              ...
              <div>
                ...
                <input type="text" name="title" required placeholder="Məs. Riyaziyyat"
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white border border-gray-150 rounded-2xl max-w-md w-full p-6 shadow-2xl animate-in zoom-in duration-200">
            <div className="flex justify-between items-center mb-5">
              ...
              <button onClick={() => { setShowEditModal(false); setSelectedSubject(null); }} className="text-gray-400 hover:text-gray-650 cursor-pointer">✕</button>
            </div>
            <Form method="post" className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Fənnin Adı</label>
                <input type="text" name="title" defaultValue={selectedSubject.title} required
                  className="w-full bg-slate-50 border border-gray-250 rounded-xl px-4.5 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-650 outline-none" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Eyniləşdirici / Slug</label>
                <input type="text" name="identify" defaultValue={selectedSubject.identify}
                  className="w-full bg-slate-50 border border-gray-250 rounded-xl px-4.5 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-650 outline-none font-mono" />
              </div>
              <div className="flex gap-3 justify-end mt-6">
                <button type="button" onClick={() => { setShowEditModal(false); setSelectedSubject(null); }}
                  className="py-2.5 px-4 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-semibold rounded-xl cursor-pointer">İmtina</button>
                <button type="submit" disabled={navigation.state === "submitting"}
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white border border-gray-150 rounded-2xl max-w-sm w-full p-6 shadow-2xl animate-in zoom-in duration-200">
            ...
            <Form method="post" className="flex gap-3 justify-end mt-6">
              ...
              <input type="hidden" name="id" value={selectedSubject.id} />
              <button type="button" onClick={() => { setShowDeleteConfirm(false); setSelectedSubject(null); }}
                className="py-2.5 px-4 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-semibold rounded-xl cursor-pointer">İmtina</button>
              <button type="submit" disabled={navigation.state === "submitting"}
